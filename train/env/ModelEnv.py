...
class ModelEnv(gym.Env):
    ...
    def final_action_wall(self):
        while True:
            ...
            model, cycles, avg_util = set_folding(model, self.args.output_dir, self.args.board_file, self.args.freq, self.args.target_fps, self.args.slr)
    ...
    def maximum_fps(self):
        ...
        model = create_dataflow_partition(model)
        model = specialize_layers(model, self.args.fpga_part)
        model, cycles, _ = set_folding(model, self.args.output_dir, self.args.board_file, self.args.freq, self.args.target_fps, self.args.slr)
        fps = self.args.freq * 10**6 / cycles
